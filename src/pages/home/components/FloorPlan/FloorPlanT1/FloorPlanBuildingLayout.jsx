...
const FloorPlanBuildingLayout = ({
  ...
  const FloorButton = ({ customStyle, i }) => {
    return (
      ...
      >
        {data.floors[i]?.btnTitle}
        <div className="relative">
          <div
            style={{ backgroundColor: data.floors[i]?.color }}
          ...
          >
            <p className="text-smaller font-semibold text-cyan-900">
              {data.floors[i]?.btnTitle}
            </p>
            <p className="font-bold text-med">
              {data.floors[i]?.bedRoomNumber} {t("floorBedroom")}
            </p>
          </div>
        </div>
      </button>
